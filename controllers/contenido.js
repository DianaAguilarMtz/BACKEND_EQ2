...
exports.postActualizarContenido = async (req, res) => {
    //Filtro y cambio
    try {
        const dato = await Contenido.findOneAndUpdate(
...
exports.postBorrarContenido = async (req, res) => {
    const data = await Contenido.findOneAndDelete({
        tipo: req.body.tipo,
        titulo: req.body.titulo,
    });
    if(data===null){
...
exports.postActualizarPlaylist = async (req, res) => {
    try {
        const data = await Contenido.findOneAndUpdate(
            { playlist: req.body.playlist, titulo: req.body.titulo },
            { playlist: "none" }
        );
        if(data===null){
            res.status(422).json({
